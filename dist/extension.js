...
`,n+=`| :--- | :--- | :--- | :--- | :--- | :--- |
`;for(let d of e.sessions.slice(0,50)){let g=new Date(d.startTime).toLocaleString(a),v=d.title.replace(/\|/g,"\\|"),k=d.modelsUsed.join(", ");n+=`| ${g} | ${v} | ${k} | ${this.formatNumber(d.totalTokens)} | ${this.formatDuration(d.activeTimeSeconds)} | $${d.costUSD.toFixed(4)} |
...
`,i+=`| :--- | :--- | :--- | :--- | :--- | :--- |
`;for(let n of e.sessions.slice(0,50)){let d=new Date(n.startTime).toLocaleString(a),g=n.title.replace(/\|/g,"\\|"),v=n.modelsUsed.join(", ");i+=`| ${d} | ${g} | ${v} | ${this.formatNumber(n.totalTokens)} | ${this.formatDuration(n.activeTimeSeconds)} | $${n.costUSD.toFixed(4)} |
...
</html>`}};var $=N(require("vscode")),de=N(require("https")),z=N(require("fs")),Me=N(require("path")),$e=N(require("os"));function Te(r,e){let t=r.replace(/^v/,"").split(".").map(Number),o=e.replace(/^v/,"").split(".").map(Number);for(let s=0;s<Math.max(t.length,o.length);s++){let c=t[s]||0,l=o[s]||0;if(l>c)return!0;if(l<c)return!1}return!1}var ue="trinhhaox",pe="acost",Oe=`https://api.github.com/repos/${ue}/${pe}/releases/latest`,He=`https://api.github.com/repos/${ue}/${pe}/tags`;async function me(r,e,t=!1){try{let o=r.extension.packageJSON.version||"1.0.0",c=(e.language||"vi")==="en";de.get(Oe,{headers:{"User-Agent":"Acost-Extension",Accept:"application/vnd.github.v3+json"}},a=>{let i="";a.on("data",n=>{i+=n}),a.on("end",async()=>{if(a.statusCode===200)try{let n=JSON.parse(i),g=(n.tag_name||"").replace(/^v/,"");if(g&&Te(o,g)){let v="";if(Array.isArray(n.assets)){let k=n.assets.find(R=>R.name&&R.name.endsWith(".vsix"));k&&k.browser_download_url&&(v=k.browser_download_url)}await Ee(g,n.html_url,v,c)}else if(t){let v=c?`Acost is up to date (v${o}).`:`Acost \u0111\xE3 l\xE0 phi\xEAn b\u1EA3n m\u1EDBi nh\u1EA5t (v${o}).`;$.window.showInformationMessage(v)}}catch{De(o,c,t)}else De(o,c,t)})}).on("error",a=>{t&&$.window.showErrorMessage(c?`Failed to check for updates: ${a.message}`:`L\u1ED7i khi ki\u1EC3m tra c\u1EADp nh\u1EADt: ${a.message}`)})}catch(o){t&&$.window.showErrorMessage(`Update check error: ${o?.message||o}`)}}function De(r,e,t){de.get(He,{headers:{"User-Agent":"Acost-Extension",Accept:"application/vnd.github.v3+json"}},s=>{let c="";s.on("data",l=>{c+=l}),s.on("end",async()=>{if(s.statusCode===200)try{let l=JSON.parse(c);if(Array.isArray(l)&&l.length>0){let i=(l[0].name||"").replace(/^v/,"");if(i&&Te(r,i)){let n=`https://github.com/${ue}/${pe}/releases`;await Ee(i,n,"",e);return}}}catch{}if(t){let l=e?`Acost is up to date (v${r}).`:`Acost \u0111\xE3 l\xE0 phi\xEAn b\u1EA3n m\u1EDBi nh\u1EA5t (v${r}).`;$.window.showInformationMessage(l)}})}).on("error",()=>{})}async function Ee(r,e,t,o){let s=o?"\u26A1 1-Click Update":"\u26A1 C\u1EADp Nh\u1EADt T\u1EF1 \u0110\u1ED9ng",c=o?"\u{1F310} Open GitHub":"\u{1F310} Xem B\u1EA3n Ph\xE1t H\xE0nh",l=o?"Later":"\u0110\u1EC3 Sau",a=o?`\u{1F680} Acost v${r} is now available! Would you like to update?`:`\u{1F680} \u0110\xE3 c\xF3 phi\xEAn b\u1EA3n m\u1EDBi Acost v${r}! B\u1EA1n c\xF3 mu\u1ED1n c\u1EADp nh\u1EADt ngay kh\xF4ng?`,i=t?[s,c,l]:[c,l],n=await $.window.showInformationMessage(a,...i);n===s&&t?await We(t,r,o):n===c&&$.env.openExternal($.Uri.parse(e))}async function We(r,e,t){let o=t?`Downloading Acost v${e}...`:`\u0110ang t\u1EA3i b\u1EA3n c\u1EADp nh\u1EADt Acost v${e}...`;await $.window.withProgress({location:$.ProgressLocation.Notification,title:o,cancellable:!1},async s=>{let c=Me.join($e.tmpdir(),`acost-${e}-${Date.now()}.vsix`);try{await Ge(r,c,g=>{s.report({message:`${g}%`})});let l=t?"Installing update...":"\u0110ang c\xE0i \u0111\u1EB7t c\u1EADp nh\u1EADt...";s.report({message:l});let a=$.Uri.file(c);await $.commands.executeCommand("workbench.extensions.installExtension",a);let i=t?"\u{1F504} Reload Window":"\u{1F504} Kh\u1EDFi \u0110\u1ED9ng L\u1EA1i",n=t?`\u{1F389} Acost has been updated to v${e}! Please reload to apply changes.`:`\u{1F389} \u0110\xE3 c\u1EADp nh\u1EADt Acost l\xEAn v${e} th\xE0nh c\xF4ng! Vui l\xF2ng kh\u1EDFi \u0111\u1ED9ng l\u1EA1i \u0111\u1EC3 \xE1p d\u1EE5ng.`;await $.window.showInformationMessage(n,i)===i&&await $.commands.executeCommand("workbench.action.reloadWindow")}catch(l){let a=t?`Failed to automatically update: ${l.message}. Please download manually.`:`Kh\xF4ng th\u1EC3 t\u1EF1 \u0111\u1ED9ng c\u1EADp nh\u1EADt: ${l.message}. Vui l\xF2ng t\u1EA3i th\u1EE7 c\xF4ng t\u1EEB GitHub.`,i=t?"Download Manually":"T\u1EA3i Th\u1EE7 C\xF4ng";await $.window.showErrorMessage(a,i)===i&&$.env.openExternal($.Uri.parse(`https://github.com/${ue}/${pe}/releases`))}finally{try{z.existsSync(c)&&z.unlinkSync(c)}catch{}}})}function Ge(r,e,t){return new Promise((o,s)=>{let c=l=>{de.get(l,{headers:{"User-Agent":"Acost-Extension"}},i=>{if((i.statusCode===301||i.statusCode===302||i.statusCode===307)&&i.headers.location){c(i.headers.location);return}if(i.statusCode!==200){s(new Error(`Server returned HTTP ${i.statusCode}`));return}let n=parseInt(i.headers["content-length"]||"0",10),d=0,g=z.createWriteStream(e);i.pipe(g),i.on("data",v=>{if(d+=v.length,n>0&&t){let k=Math.round(d/n*100);t(k)}}),g.on("finish",()=>{g.close(),o()}),g.on("error",v=>{z.unlink(e,()=>{}),s(v)})}).on("error",i=>{z.unlink(e,()=>{}),s(i)})};c(r)})}var Q,xe,Pe,S=null,x,ge=null,he=null,fe=null,ne,Ne="all";function Ae(){let r=b.workspace.getConfiguration("acost"),e=b.workspace.getConfiguration("antigravityCost"),t=(b.env.language.startsWith("vi"),"vi");return{language:r.get("language",e.get("language",t)),currency:r.get("currency",e.get("currency","USD")),vndExchangeRate:r.get("vndExchangeRate",e.get("vndExchangeRate",25500)),markupMultiplier:r.get("markupMultiplier",e.get("markupMultiplier",2.5)),humanHourlyRate:r.get("humanHourlyRate",e.get("humanHourlyRate",25)),customPricing:r.get("customPricing",e.get("customPricing",{}))}}function Ie(){if(ne&&ne!=="CURRENT")return ne==="ALL"?void 0:ne;let r=b.workspace.workspaceFolders;if(r&&r.length>0)return r[0].uri.fsPath}async function U(r=!1,e,t){e!==void 0&&(ne=e),t!==void 0&&(Ne=t);let o=Ie();S=await Q.scanWorkspace(o,Ne),xe.update(S,x),Pe.updateReport(S,x),r&&S&&(x.language==="en"?b.window.showInformationMessage(`Acost [${S.projectName}]: Scanned ${S.totalSessions} sessions (${C.formatNumber(S.totalTokens)} tokens, ~$${S.totalCostUSD.toFixed(3)})`):b.window.showInformationMessage(`Acost [${S.projectName}]: \u0110\xE3 qu\xE9t ${S.totalSessions} sessions (${C.formatNumber(S.totalTokens)} tokens, ~$${S.totalCostUSD.toFixed(3)})`))}async function te(r="markdown"){let e=X(x.language);if(!S||S.totalSessions===0){b.window.showWarningMessage(x.language==="en"?"No cost data available to export.":"Ch\u01B0a c\xF3 d\u1EEF li\u1EC7u chi ph\xED \u0111\u1EC3 xu\u1EA5t b\xE1o c\xE1o.");return}let o=Ie()||b.Uri.file(process.cwd()).fsPath,s="PROJECT_VALUATION_REPORT.md",c="";r==="html"?(s="PROJECT_VALUATION_REPORT.html",c=C.generateHtml(S,x)):r==="json"?(s="project_cost_data.json",c=C.generateJson(S)):c=C.generateMarkdown(S,x);let l=ve.join(o,s);try{J.writeFileSync(l,c,"utf8");let a=e.openFile,i=e.reportExportSuccess.replace("{filename}",s);if(await b.window.showInformationMessage(i,a)===a){let d=await b.workspace.openTextDocument(b.Uri.file(l));await b.window.showTextDocument(d)}}catch(a){b.window.showErrorMessage(e.exportError.replace("{err}",a?.message||a))}}function ze(){let r=null,e=()=>{r&&clearTimeout(r),r=setTimeout(()=>{U()},1500)};try{let t=ve.join(ye.homedir(),".gemini","antigravity-ide","brain");J.existsSync(t)&&(he=J.watch(t,{recursive:!0},(o,s)=>{s&&s.endsWith("transcript.jsonl")&&e()}))}catch{}try{let t=ve.join(ye.homedir(),".claude","projects");J.existsSync(t)&&(fe=J.watch(t,{recursive:!0},(o,s)=>{s&&s.endsWith(".jsonl")&&e()}))}catch{}}function qe(r){if(x=Ae(),Q=new ae(x),xe=new ce,Pe=new le(r.extensionUri,x,async(t,o)=>{await U(!0,t,o)},async t=>{await te(t)},async t=>{x={...x,...t},Q.updateConfig(x),S&&await U()}),r.subscriptions.push(b.window.registerWebviewViewProvider("acost.sidebar",Pe)),r.subscriptions.push(xe),r.subscriptions.push(b.commands.registerCommand("acost.refresh",async()=>{await U(!0)})),r.subscriptions.push(b.commands.registerCommand("acost.exportReport",async()=>{await te("markdown")})),r.subscriptions.push(b.commands.registerCommand("acost.exportHtmlReport",async()=>{await te("html")})),r.subscriptions.push(b.commands.registerCommand("acost.searchProject",async()=>{let t=X(x.language);if((!S||!S.allProjects||S.allProjects.length===0)&&await U(),!S||!S.allProjects||S.allProjects.length===0){b.window.showWarningMessage(x.language==="en"?"No projects found.":"Ch\u01B0a t\xECm th\u1EA5y d\u1EF1 \xE1n n\xE0o.");return}let o=x.language==="en",s=x.currency==="VND",c=[];c.push({label:`$(folder) ${o?"Current Project":"D\u1EF1 \xC1n Hi\u1EC7n T\u1EA1i"}`,description:`(${S.projectName})`,detail:`\u{1F4CD} ${S.workspacePath}`,wsPath:"CURRENT"}),c.push({label:`$(globe) ${o?"All Projects on Machine":"T\u1EA5t C\u1EA3 D\u1EF1 \xC1n Trong M\xE1y"}`,description:`(${S.allProjects.length} projects)`,detail:`\u{1F310} ${o?"Summarize token & cost across all workspaces":"T\u1ED5ng h\u1EE3p token & chi ph\xED c\u1EE7a t\u1EA5t c\u1EA3 c\xE1c d\u1EF1 \xE1n"}`,wsPath:"ALL"});for(let a of S.allProjects){let i=s?`${C.formatNumber(a.totalCostVND)} \u20AB`:`$${a.totalCostUSD.toFixed(2)}`,n=`${C.formatNumber(a.totalTokens)} tok`,d=`${a.totalSessions} sess`;c.push({label:`$(file-directory) ${a.projectName}`,description:`\u{1F4B0} ${i}  \u2022  \u26A1 ${n}  \u2022  \u{1F4DD} ${d}`,detail:`\u{1F4CD} ${a.workspacePath}`,wsPath:a.workspacePath})}let l=await b.window.showQuickPick(c,{placeHolder:o?"\u{1F50D} Search projects by name or directory path...":"\u{1F50D} T\xECm ki\u1EBFm d\u1EF1 \xE1n theo t\xEAn ho\u1EB7c \u0111\u01B0\u1EDDng d\u1EABn th\u01B0 m\u1EE5c...",matchOnDescription:!0,matchOnDetail:!0});l&&(await U(!0,l.wsPath),b.commands.executeCommand("acost.sidebar.focus"))})),r.subscriptions.push(b.commands.registerCommand("acost.menu",async()=>{let t=X(x.language),o=[{id:"search_project",label:"$(search) T\xECm Ki\u1EBFm D\u1EF1 \xC1n (Search Projects)",description:"T\xECm nhanh theo t\xEAn, token, chi ph\xED ho\u1EB7c \u0111\u01B0\u1EDDng d\u1EABn"},{id:"dashboard",label:t.menuOpenDashboard,description:t.menuOpenDashboardDesc},{id:"refresh",label:t.menuRefresh,description:t.menuRefreshDesc},{id:"export_md",label:t.menuExportMd,description:t.menuExportMdDesc},{id:"export_html",label:t.menuExportHtml,description:t.menuExportHtmlDesc},{id:"check_update",label:t.menuCheckUpdate,description:t.menuCheckUpdateDesc},{id:"toggle_currency",label:t.menuToggleCurrency,description:`Current: ${x.currency}`},{id:"toggle_language",label:t.menuToggleLanguage,description:`Current: ${x.language==="en"?"\u{1F1EC}\u{1F1E7} English":"\u{1F1FB}\u{1F1F3} Ti\u1EBFng Vi\u1EC7t"}`},{id:"settings",label:t.menuSettings,description:t.menuSettingsDesc}],s=await b.window.showQuickPick(o,{placeHolder:`Acost - AI Cost & Valuation (${x.language.toUpperCase()})`});if(s)switch(s.id){case"search_project":b.commands.executeCommand("acost.searchProject");break;case"dashboard":b.commands.executeCommand("acost.sidebar.focus");break;case"refresh":await U(!0);break;case"export_md":await te("markdown");break;case"export_html":await te("html");break;case"check_update":await me(r,x,!0);break;case"toggle_currency":{let c=x.currency==="USD"?"VND":"USD";x.currency=c,Q.updateConfig(x),await U(),b.window.showInformationMessage(x.language==="en"?`Switched currency to: ${c}`:`\u0110\xE3 \u0111\u1ED5i \u0111\u01A1n v\u1ECB ti\u1EC1n t\u1EC7 sang: ${c}`);break}case"toggle_language":{let c=x.language==="vi"?"en":"vi";x.language=c,Q.updateConfig(x),await U(),b.window.showInformationMessage(c==="en"?"Switched language to English \u{1F1EC}\u{1F1E7}":"\u0110\xE3 \u0111\u1ED5i ng\xF4n ng\u1EEF sang Ti\u1EBFng Vi\u1EC7t \u{1F1FB}\u{1F1F3}");break}case"settings":b.commands.executeCommand("workbench.action.openSettings","acost");break}})),r.subscriptions.push(b.workspace.onDidChangeConfiguration(async t=>{(t.affectsConfiguration("acost")||t.affectsConfiguration("antigravityCost"))&&(x=Ae(),Q.updateConfig(x),await U())})),r.subscriptions.push(b.workspace.onDidChangeWorkspaceFolders(async()=>{await U()})),ze(),setTimeout(()=>{U()},1e3),b.workspace.getConfiguration("acost").get("autoCheckUpdates",b.workspace.getConfiguration("antigravityCost").get("autoCheckUpdates",!0))){setTimeout(()=>{me(r,x,!1)},3e3);let t=2*60*60*1e3,o=setInterval(()=>{me(r,x,!1)},t);r.subscriptions.push({dispose:()=>clearInterval(o)})}ge=setInterval(()=>{U()},6e4)}function Je(){ge&&(clearInterval(ge),ge=null),he&&(he.close(),he=null),fe&&(fe.close(),fe=null)}0&&(module.exports={activate,deactivate});
